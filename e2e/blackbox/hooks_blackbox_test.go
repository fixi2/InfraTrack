...
func TestHooksInstallUninstallIdempotentOnTempProfile(t *testing.T) {
	...
	if strings.Count(content, "# >>> infratrack hooks >>>") != 1 {
	...
	if strings.Contains(content2, "# >>> infratrack hooks >>>") {
...
func TestHookRecorderAntiRecursion(t *testing.T) {
	...
	h.mustRun("hook", "record", "--command", "infratrack status", "--exit-code", "0", "--duration-ms", "5", "--cwd", h.workDir)
	...
	if strings.Contains(runbook, "infratrack status") {
		t.Fatalf("anti-recursion violated: infratrack command captured")
	}
}
